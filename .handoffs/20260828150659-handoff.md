...
- 待重探的 [?]: 见下方标记
- 先读: `CLAUDE.md`（hook/日志/lint 约定）+ `ABS_LEVEL_DESIGN.md`（本次最大成果的全文）+ `TC_LEVEL_DESIGN.md` §10（TC 四轮演化史）+ §2 的触摸现象原话（用户原话，上次丢过一次，禁止再丢；**出处: `.handoffs/20260827234945-handoff.md` §2**——指针曾断链三代，2026-08-28 追回锚定）
