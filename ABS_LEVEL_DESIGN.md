...
- **模式**:`默认 / 自定义`(同 TC)。**每次从默认切到自定义时弹警示**(标题"⚠建议调整最大制动压力"居中 + 正文左对齐 + "我已了解"按钮——减弱干预后重刹易锁死,提醒配合下调最大制动压力)。
- **旋钮 1 · 干预强度**(自定义卡片内,b 绝对值覆写;用户规格档名"关闭 ABS、低、中等、高、最高(默认)"):
  ...
  描述文案:"修改干预方波平均"。b<1 恒有泄压相位(native clamp 上限 0.9,不允许持续锁死——"完全锁死"留给关闭档,行业同款)。
- **旋钮 2 · 最大制动压力**(ABS 下方**独立项**,不在自定义卡片内;T_b 等比缩放,F1 官方游戏同名术语):

  | 值 | T_b 缩放 | 语义 |
  |---|---|---|
  | 100%(默认) | 不写 | 原厂基数 |
  | 0-100% **无级** | ×设定值 | 调低后关 ABS 锁死倾向同步下降;**等比缩放非截断——踏板响应曲线/纵轴完全不受影响**(absPressure 独立于 absMode 生效,默认/关闭档下也修"关 ABS 秒锁死")。首版 75% 下限太温和无法观察(见 §6.3),改 0-100%(0% = T_b 清零,高速段 F_base→0,观察信号极强) |

  描述文案:"调整游戏制动摩擦扭矩上限"。下限 75%(用户规格;native clamp 同步)。
