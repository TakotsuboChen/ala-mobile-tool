 ...
 * 用自己 context 写自己 externalFilesDir → OverlayManager 读同一路径生效。
 *
 * 首次安装/首次启动时 receiver 还没注册（要等 onPackageReady），读到默认值；
 * 用户改一次配置后重启游戏，receiver 接收并写入，之后 toggle 即生效。
 */
class ConfigReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        try {
            ...
            file.writeText(json)
            Log.i(TAG, "ConfigReceiver: wrote ${json.length} bytes to ${file.absolutePath}")
        } catch (e: Throwable) {
