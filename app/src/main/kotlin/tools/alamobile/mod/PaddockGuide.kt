...
object PaddockGuide {
     ...
     * v2（2026-09-18）：积分公式加入"辅助加成"与"零辅助金标"两节。
     */
...
积分按**每条赛道 × 每个游戏版本**分别计算，名次由该赛道的最快圈速决定，再乘上**辅助加成**。基础分公式为：
...
**总积分** = 你在所有赛道、所有版本上积分（含加成）的总和。共 16 条赛道，单版本基础分满分 1600 分，两个版本全拿第一就是 3200 分。

### 4. 辅助加成是怎么算的？

跑出成绩时，模块会把**你当圈的踏板、TC、ABS 设置**一并上报，达标即可获得加成：
...
四项全达标即最高 **+50%**。加成只作用于基础分，公式为：

**加成 = 基础分 × 加成比例**（不足 1 分按 1 分算，大于 1 分四舍五入）

例如基础分 60 分、四项全达标：60 × 50% = 30 分，合计 **90 分**。基础分为 0 时没有加成。

有几点必须说明：

- **未开启自定义的 TC / ABS 按"最高档"计**，也就是不加分。只有主动进设置把它们调下来或关掉才算；
- 踏板加成按**设置**计算（只要踏板模式是单踏板或双踏板即算），与你是否临时隐藏控件无关；
- 一整圈中途改过设置的成绩会记作**缺失**，不加分也不减分；
- 早期版本上传的成绩没有配置信息，同样记为缺失，不影响名次分。

### 5. 排行榜上的金字是什么？

榜上带**金色流光**的行，表示这是"零辅助"成绩——即该圈 **TC 与 ABS 都关闭**：

- **赛道榜**：你的个人最快圈是零辅助，你的这一行就会闪烁金光。即使不是全服第一也照样发光，因为零辅助本身就是含金量；
- **积分榜**：当你有**至少一半**的个人最快圈是零辅助时，你的名字和积分会闪烁金光。总榜与版本榜分别计算，可能出现榜上有金标、另一个榜没有的情况。

辅助加成的统计口径与金字完全一致，都来自模块的上报数据。
