...
- **⚠️ 回调体内严禁浮点操作（v6 死法，2026-08-30 实机两轮实证——本次拦截器顺带毁了 ABS 档位）**：被拦截指令要重放 s0（T×b 结果写 tempBrakeF）。`SHADOWHOOK_INTERCEPT_DEFAULT` 不保存 FPSIMD——回调里 `*(float*)(wheel+0xF0) > 0.01f` 的浮点比较经 s0 执行，污染重放值：pulse 泄压帧被写入垃圾（第一轮 DEFAULT 下垃圾=0xF0 归一化值 1.0/0.88…；第二轮改 `WITH_FPSIMD_WRITE_ONLY` 垃圾变 0.0——shadowhook 的 FPSIMD 保存/恢复路径恢复进的是零，**flags 补救同样不可信**）。pulse 泄压帧扭矩≈0 → 全泄压 → 方波平均恒 0.5×F_base = 原厂 b=0 行为，**ABS 档位全线失效、手感与"默认最高档"完全一致**。修复 = 回调零浮点：0xF0 过滤改 IEEE754 位型整数比较（`*(uint32_t*)(wheel+0xF0) > 0x3C23D70A`，正 float 位型与无符号整数同序），flags 回退 DEFAULT。验证法：反汇编回调不得出现 vmov/vldr/fcmp 等浮点指令（APK 内 so 实测通过）。**症状指纹**：tempBrakeF 是扭矩量纲（百~千级），日志出现 0~1.5 的小值即 FPSIMD 污染复发。
