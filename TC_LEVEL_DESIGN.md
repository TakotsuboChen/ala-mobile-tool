...
生成时间: 2026-08-28 · 状态: **v1.5 已实装并实机定档**（2026-08-28，v1.4 四档时机全部生效后同日完成 v1.5 实机手感调优；现行值以 `ModConfig.kt` 为单一事实源，演化史见 §10）
...
- ~~**游戏原生 TC 设置 UI 本身就是 TMP_Dropdown 多档位**（settingsHandler.TractionControl 0x120，dump.cs:73917）→ 实机截图游戏内 TC 下拉框可白嫖原生档位语义（待办）~~ → **已证伪**（§2b）：settingsHandler 的 TC/ABS 设置仅手柄生效的开/关，移动端无档位可选——模块档位是移动端唯一的 TC 调节途径。
...
> 现行值（2026-08-28 用户实机定档，标定史见 §10 v1.5）；存档 value 键值未变，旧配置无需迁移。

**旋钮 2：介入时机**（写 TCLSlip，见 §3 候选 B；介入点 W_th = 1/(1−ε)）

| 档位 | 标签 | 参数 | 介入点 | 语义 |
|---|---|---|---|---|
| 0 | `较晚`（默认） | 不覆写 | 原厂 W>1.67（ε=0.40） | **不覆写字段**，切回时一次性恢复捕获基线（运行时真值 ε=0.40 / minSPD=11.0，非 ctor 默认 0.45/1.0——游戏设置无任何 TC 参数可调） |
| 1 | `较早` | ε=0.35 | W>1.54 | 配对 minSPD=8.0（约 29 km/h 起介入） |
| 2 | `非常早` | ε=0.25 | W>1.33 | 配对 minSPD=4.0（约 14 km/h 起介入） |
| 3 | `实时` | ε=0.02 | W>1.02 | 配对 minSPD=0.5，接近"一打滑就削"（保留门控≠0 语义） |

- 方向说明写入 UI：时机数值越小 = 介入越早、削减曲线越陡。
- 初值已实机定档（2026-08-28，三轮标定见 §10 v1.5；现行值以 `ModConfig.kt` 为单一事实源）；"更晚"档（ε 上调 → W>2.5）视需求再加。
- 空挡（gear==1，UI 显示 N）豁免不受这两个旋钮影响；UI 显示的"1 挡"是 gear==2，起步期间 TC 正常活跃（§2b 反汇编修正，旧"一挡豁免"结论已证伪）。
- ~~与游戏原生下拉框的融合：若实机截图证实原生档位语义，可对齐命名~~（**§2b 作废**：游戏原生根本没有 TC/ABS 档位可选，仅手柄生效的开/关）。
...
2. ~~**游戏原生档位对齐**：待实机截图游戏内 TC 下拉框；若原生有 3-4 档，考虑对齐命名或提供"原生档位+扩展档"~~（**§2b 作废**：原生无档位可选，仅手柄生效的开/关——对齐目标不存在）。
