...
- M17: Tool button rework — **done & real-device verified**. Reworked the floating "工具" overlay button from a 70dp black Button to a custom `ToolButtonView` (raw Android Canvas View) showing a rounded-rectangle (screen-height 10%) with centered App icon + draggable + long-press-into-edit-mode. `ToolButtonIcon.kt` embeds the 144×144 ldpi foreground PNG as base64 (~16KB) — `BitmapFactory.decodeResource` returns null in game process (game Resources doesn't know module R values), `PackageManager.getResourcesForApplication` throws `NameNotFoundException` (Android 11+ package visibility), so base64 embedding is the only reliable cross-process icon loading. Single-tap toggles expand/collapse, drag moves itself (not persisted, resets to default on `showOverlays`), long-press 500ms enters other-overlay edit mode. `ModConfig` gained `KEY_TOOL_POSITION`/`toolButtonPosition` field (architecture-uniform, `write()` doesn't write it, `read()` reads but doesn't depend — user wants reset-to-default each launch). Build + lint green; real-device verified (手势全过).
- M18: Multi-car + restart pedal failure fix — **ShadowHook UNIQUE mode fix done & real-device verified; AI car throttle contamination bug pending**. Root cause: ShadowHook SHARED mode inline hook trampolines are overwritten after first invocation — game's built-in pairip security framework (`com.pairip.application.Application`) protects `libil2cpp.so` code segments, shared trampoline pages get restored. All 7 hooks (setThrottle/setBrake/shiftUp/shiftDown/FixedUpdate/DrivetrainFixedUpdate/PlayerControlsUpdate) only fired ~1 time each under SHARED mode (FixedUpdate should fire 50Hz). Fix: `ShadowHook.Mode.SHARED`→`UNIQUE` in `AlaMobileModule.kt` — each hook gets independent trampoline page, pairip's page-level integrity check can't restore independent pages. Confirmed: `DrivetrainFixedUpdate` logcat count went from 1 (SHARED) to 3351 (UNIQUE). Also added: (1) `is_player_controller()` reading `IRDSCarControllInput.playerControls` field (offset 0x108, non-null = player car, `IRDSPlayerControls` only mounts on player GameObject) replacing unreliable `carPilot` (0x68) check in `proxy_set_throttle`/`proxy_set_brake`/`proxy_fixed_update`; (2) `proxy_player_controls_update` hook on `IRDSPlayerControls.Update` (RVA 0x1A62E04) continuously refreshes `g_last_controller` from `this+0x60` (carInputs) every frame — survives scene reload/restart; (3) use-after-free guards on writer thread + `pedal_set_throttle_value`/`pedal_set_brake_value`/`pedal_shift_up`/`pedal_shift_down`. **Known bug pending fix**: when throttle > 0, ALL AI cars accelerate together with the module pedal (brake unaffected, AI normal when throttle=0 or braking). Likely cause: `is_player_controller` returns true for AI cars too (AI's `IRDSCarControllInput.playerControls` may be non-null), or `proxy_fixed_update`/writer writes to AI controller. Build + lint green; real-device verified "重新开始、多人模式已生效" but AI contamination reported.
- M19: vivo/OriginOS/Android 16 + NPatch 本地模式解锁修复 — **done & vivo 云测真机验证解锁成功**。Root cause: (A) `App.onCreate` 在游戏进程的 `createOrUpdateClassLoaderLocked` 内部同步调用，`XposedServiceHelper.registerListener` + `ContentResolver.call` 干扰 `LoadedApk.getResources()` → `makeApplicationInner` NPE 闪退。Fix: 游戏进程里 `App.onCreate` 把 service binding 全部 `Handler.post` 延迟到 next main loop；`onPackageLoaded`（BillingHook.install）和 `onPackageReady`（ShadowHook.init + forceLoad + initUnlock）也全部 deferred。(B) NPatch 隔离 ClassLoader 下 `System.loadLibrary` 失败。Fix: `forceLoad` 改为 context 可空，从 `BaseDexClassLoader.pathList.dexElements` 反射拿模块 APK 路径。(C) `OnAlreadyOwned(string)` 手写 IL2CPP string `klass=NULL` 导致 `string.Equals` 比较 false。Fix: 改用 `SetUnlocked(true)` 直调（RVA 0x186E440），不需要 string，直接设 `IsUnlocked=true` + `PlayerPrefs.SetInt("AnciTuttu",1)` + `OnUnlockedChanged.Invoke()`。(D) NPatch 无 daemon，配置同步断链。Fix: `App.bindNpatchRemoteService` 主动从 NPatch 管理器 `content://top.nkbe.npatch.remote` 拿 binder；`enableUnlock` 在配置读不到时默认 true。Also: (1) `BillingManager.GetInstance` hook（RVA 0x186C958）作 Awake 兜底注入点；(2) `forceUnlockNow()` JNI one-shot 在 15s 路径调 `get_Instance()` → `SetUnlocked(true)`；(3) native 日志双写 `ala_native.log`（NPatch 日志目录，用户不用 adb）；(4) manifest `<queries>` 声明 `top.nkbe.npatch`。Build + lint green; vivo 云测 + Meizu 20（无谷歌账号）真机验证解锁成功。
- M20: 原生 TC/ABS 开关 + 手动换挡 UI 启用 + 配置页图标自定义 — **TC 已真机验证生效；ABS 未生效（HandleABS 是死代码，开关已暂时注释）；手动换挡待重做（开关已暂时注释）；配置页 5 项图标已自定义**。新增 `enableTc`/`enableAbs` 配置字段（默认 true）全链路同步（ModConfig→ConfigUiState→ConfigReceiver→NativeBridge.setTcAbs→native g_config）。UI: 功能开关卡片新增「牵引力控制」「防抱死系统」开关（ABS 开关已注释掉，待找到正确 ABS 入口后恢复），注释掉「自动 DRS」；「手动换挡」开关已注释掉（DoGearShifting hook 导致出不了 P 房，待重做后恢复）。Native: hook `TractionFilter`（RVA 0x1A570C8）方法入口，关闭时直接返回原始 accel（**TC 已真机验证：关掉后大油门明显更滑**）。hook `HandleABS`（RVA 0x1A5763C）——**全 so 搜不到任何 `bl 0x1a5763c` 调用，HandleABS 是死代码**；写 `absEnable=0` 也无效。新增 `DoGearShifting` hook——**导致车出不了 P 房，已回退为直接调 orig**。配置页图标：新增 `CustomIcons.kt`（TcIcon/PedalsIcon/BrakeCurveIcon 用 Inkscape flatten 纯 path，AbsIcon 只保留刹车盘圆+弧 stroke，GearboxIcon 两齿轮 45° 对角线环+齿帽分离方案）；TC/踏板/刹车曲线换自定义图标，油门曲线保留 `Icons.Rounded.Speed`，刹车方向反转图标改 `Icons.Rounded.Flip`，TC summary 改"启用游戏原生 TC"。Build + lint green。
