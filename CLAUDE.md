...
**Log pulling (实机日志拉取)**: game-side logs live at `/sdcard/Android/media/<游戏包>/ala_tool.log` (Java) + `ala_tool_native.log` (native) + `ala_tool_crash_native.log`（native 崩溃自捕），where `<游戏包>` is **either** package name above depending on which build the user runs. Device log file timestamps are the fastest way to tell which build produced a given log. ⚠️ 2026-09-14 从 `Android/data/<pkg>/files/` 迁到 `Android/media/<pkg>/`：media 不在 scoped storage 受限区，模块 App 持 AFA 可**跨包直读导出**（游戏闪退后仍能导出）；旧目录的日志为历史残留，新日志只写 media。
...
├── Logger / LogExporter      # Unified logging (unconditional file output) + ShareSheet export（media 直读，无门控）
...
- ⚠️ **严禁整段 `md.disasm(detail=True)`**（31MB 代码段全量会 OOM 整机，已致 WSL2 宿主重启）；一律 `disasm_lite` 流式 + 外层 `ulimit -v 4194304`。方法代码在 `il2cpp` 节（~31MB）不在 `.text`（0x2c3160），动手前 `readelf -SW` 反查所属节。
...
- **编辑模式（长按工具图标出入）的几条非显然约定**，改前先读：① **编辑层必须铺满整屏**（`OverlayEditView` 的布局尺寸 = 屏幕，被编辑控件矩形是它内部的 `RectF`）——若让编辑层尺寸 = 控件尺寸，四角圆点圆心落在 view 自身布局边界上、圆面越界被裁成 1/4 个圆，靠父容器 `clipChildren=false` 修不好（父链每层都要放行，实测只修好几个角）。② **触摸三路分流**（2026-09-17 定稿）：DOWN 落本层矩形（含角命中圈）→ 拖拽/缩放；落空白（`isBlankArea` 聚合判定，遍历**全部**编辑层矩形 + 工具按钮矩形——触摸只会派给 z 序最高层，单层自判会把兄弟矩形当空白吃掉）→ **消费事件**（编辑模式下游戏全屏禁触的实现点）；落兄弟矩形或工具按钮 → return false 穿透（工具按钮穿透是退出编辑模式的唯一出口，被吃掉用户锁死）。③ **进出编辑模式都必须走 `applyOverlayVisibility()`**（`VISIBLE = editMode \|\| overlaysVisible`）——编辑模式不改 `overlaysVisible`，若某条路径单独按它判定，会出现"退出后控件还留着"或"编辑中改参数控件消失"。④ **退出编辑模式不得重建控件**（旧实现进出都 `removeGamingOverlays()+addGamingOverlays()`，退出时淡出动画作用在刚出生就 GONE 的新层上 = 瞬间恢复无渐亮）。⑤ 编辑模式**屏蔽工具图标单击**（单击=展开/折叠，会与编辑状态打架），唯一进出口都是长按。⑥ 整屏变暗层 z 序**不能插 index 0**（Unity 的 SurfaceView 被合成器提到窗口 View 之上，index 0 在它下面盖不住），必须紧贴游戏内容之后、自家控件之前。⑦ 踏板控件用 `saveLayer` 承担透明度设置，**任何不应跟随透明度的绘制（如编辑模式的「油门」/「刹车」标识）必须放在 `restoreToCount` 之后**。⑧ **dim/hint 层不随 rebuild 摘除、也绝不清字段引用**（`removeGamingOverlays` 有三个调用方：toggle/进场/rebuild——rebuild 时若清引用，`ensureDimView`/`ensureHintView` 的 takeIf 复用失效会**新建第二份层**，旧层 alpha=1 挂在树上无人管理 = 退出后黑屏+提示常驻，2026-09-17 实测）。防抖靠复用：rebuild 命中旧层，alpha 已是 1，1→1 空动画天然不重播渐暗；初值必须恒 0，**不能写 `alpha = if (editMode) 1f else 0f`**（进场路径 toggleEditMode 先置位 editMode 再走到这，条件恒命中 1f 分支 → 新层初值 1 + 1→1 空动画 = 进场闪黑，同日实测）。⑨ **alpha 动画收尾禁用 `alpha == 0f` 判据**：`postDelayed(时长)` 与动画末帧（Choreographer 调度）竞序差 0~1 帧，alpha≈0.03 恒不为 0 → 收尾永不执行（表现为"退出后编辑层 alpha≈0 常驻吃触摸，踏板仍可拖拽"）——守卫语义状态（`!editMode`），不守卫动画视觉终值。⑩ **长按空白 2s 重置当前显示踏板**（`LONG_PRESS_RESET_MS`，提示文案引用常量联动），取消阈值 **3×touchSlop 欧氏距离**——touchSlop（≈8dp）是"拖拽开始"语义，手指静止的生理抖动逐帧累积必超 1×slop，裸用会"经常触发不了长按"（touchSlop 本身仍是控件内拖拽升级的判据，不改动）。
- miuix dialogs (`OverlayDialog`) must stay mounted in the composition tree and be driven by their `show` param — mounting under `if (visible) { ... }` skips the exit animation (dialog vanishes instantly). Follow the SupportDialog/EulaDialog pattern: **two variables** — `mounted` gates the composition-tree mount, `show` drives the param; close = flip `show=false` (plays exit animation), and `onDismissFinished` flips `mounted=false` to unmount. A single variable serving both roles skips the exit animation. Dialogs also follow the layout rule: **title centered + bold, body left-aligned** (never center body text). ⚠️ **弹窗内有"可滚动正文 + 固定按钮"时必须给正文 `Modifier.weight(1f, fill = false)`**：`DialogContentLayout` 对卡片有 `heightIn(max = 2/3 屏)` 硬上限，内容超限时 `Column` 会把所有"无 weight 子项"（正文 `heightIn` 上限 + 按钮 `defaultMinSize` 最小高）**按比例压缩**——按钮 40dp 最小高被挤成十几 dp 的窄条（3392x2400 平板实测 38px ≈ 14.5dp）。`weight` 让正文成为可让渡空间的一方（超限先收缩并滚动，按钮保高），`fill = false` 保证内容短时弹窗仍按内容收拢、不撑满 2/3 屏。
...
- `ui/CustomIcons.kt` 的 `svgIconMulti(name, viewportW, viewportH, paths, defaultWidth, defaultHeight)` 把 SVG 转 `ImageVector`，两条坑：① **SVG `viewBox` 是四元组** `minX minY width height`，但 Compose `ImageVector` **不支持非零 viewport 原点**（`viewportWidth/Height` 只定义 `0..W`）。带偏移的 viewBox（如 `"5 25.75 90 48.49"`）必须**把 path 起点的绝对坐标减去 (minX, minY)**，否则超出 `0..viewportH` 的部分被裁掉——实测表现为"图标只显示一半"；相对坐标命令（小写 `c`/`l`）不受平移影响，可原样保留。改完用 `rsvg-convert` 渲染原图与平移后图做像素对拍（`compare -metric AE`）验证。② 图形非正方形时 **intrinsic 尺寸必须同比例**（默认 24×24 会非等比拉伸），用 `defaultWidth/defaultHeight` 传对应比例。
...
- `app/src/main/kotlin/tools/alamobile/mod/update/ForceUpdateGate.kt` — 启动门控（2026-09-10 扩展）：判定红线 = **确认游戏/模块版本之前完全零 Hook**。四态：游戏版本不匹配（本地同步判定，`isSupportedVersion`，无 fail-open——错版本装 hook = 开屏闪退实证）/ 模块落后最新 Release（缓存秒判+后台检查，离线 fail-open）/ 双失配 / **未登录围场**（ConfigProvider 权威判定，fail-closed，更新失配优先于登录）；激活后循环 Toast（文案四态动态取）。⚠️ scope 里有非游戏包（GMS/WebView）——`evaluate` 入口必须按包名放行非游戏进程，否则误激活会在 GMS 进程弹 Toast（实机踩过）。`isVerdictDone` 语义 = 游戏版本已确认 **且** 模块判定出结果；context 为 null 时 verdict 不放行，hook 路径 200ms 主线程轮询重试（绝不阻塞）。`isLoginVerdictDone` 是 early unlock 路径的额外守门（登录判定走阻塞 IPC，在途时 verdictDone 拉回 false）；⚠️ `evaluateLoginGate` 禁止用 `loginVerdictDone` 初始值 true 做 early return——那会让查询永不发起、门控永不激活（短路教训实测）。登录门控 UI 侧 = `LoginGateCoordinator`（弹窗优先级最低，EULA/更新/激活弹窗埋点汇报）+ MainScreen `gateLocked` 状态机（**点「去登录」之后才锁页**，弹窗期间可自由浏览）。**放行侧另有两态启动 Toast**（`announcePaddockConnection`，2026-09-10）：本地登录态确认后异步 `fetchMe` — 通 → 「已成功连接到围场，欢迎 x 号车手 xxx!」（regSeq/username）；连不上（断网/非 200/token 失效）→ 网络异常+成绩本地暂存自动补传提示；⚠️ 必须在 `verdictDone` 置位**之后**独立线程跑，不得阻塞判定（否则 early unlock 错过 BillingManager.Awake ≈2s 窗口 → 解锁失败）。
- `app/src/main/kotlin/tools/alamobile/mod/ui/LoginGateCoordinator.kt` — 登录门控 UI 侧协调器（弹窗优先级最低）：EULA/更新/激活弹窗各自埋点汇报进度，`canGate(loggedIn)` 聚合后放行。⚠️ **更新检查是登录门控的关键路径**：`updateCheckDone` 由概览页 `LaunchedEffect(eulaAccepted)` 里的 `runStartupUpdateCheck` 置位（**不得内联在 `LaunchedEffect(Unit)` 的 `if(eulaAccepted)` 里**——全新安装时 EULA 首帧未同意，Unit effect 跑完即死、同意后无 effect 重跑 → 检查永不触发 → 门控弹窗永不出现，2026-09-13 实测）。⚠️ 检查期间置 `updateBlocking=true` 挡门控（保「更新优先于登录」），出结果即解锁（有更新则留给 UpdateDialog 的 `onRequestClose`）。
- `app/src/main/kotlin/tools/alamobile/mod/update/UpdateChecker.kt` — GitHub Release 检查更新。**多源并发竞速**（官方 api.github.com + 3 个实测可达镜像，见 `sources`）：⚠️ 竞速协程必须 launch 到**独立 `CoroutineScope`** 而非 `coroutineScope{}`——后者会 join 全部子协程才返回，提前取赢家只是跳出内层，函数仍等所有败者跑完 → **总耗时恒等于最慢源**（2026-09-13 实测五轮 1770/4015/1705/1959/1145ms 全部等于该轮最慢源；改独立 scope + `raceScope.cancel()` 后 609/782ms = 最快源耗时）。OkHttp `execute()` 不可中断，败者线程靠自身超时自然消亡（connect 8s / read 12s）但不阻塞返回。⚠️ 诊断日志禁用 `result::class.simpleName`——release 构建 R8 混淆成 `qm2` 这类乱码，用稳定字面量。
...
- `app/src/main/kotlin/tools/alamobile/mod/overlay/PedalOverlayView.kt` — dual-zone pedal。**双踏板跨 view 仲裁必须"重建式"**：每个 view 记自己的曲线输出（`sharedThrottleMapped`/`sharedBrakeMapped`），仲裁时从两者重建完整一对再按 `PedalPriority` 裁决。"只写自己、顺手把对方置 0"的单向写法**屏蔽得了、解屏蔽做不到**——策略不再命中时没有对方的值可恢复，对方 view 若无新触摸事件就永远停在 0（2026-09-14 实机定案：踩住油门点刹，松开后油门卡 0 直到手指再动，"松刹车后油门延迟恢复甚至一直不给油"）。**抬脚（`resetPedalState`）后必须 `rearbitrateAfterReset()` 主动重算并送出整对输入**，恢复不能依赖另一踏板的下一次触摸事件；sentinel（`lastSent*`/`lastDrawn*`）**按通道清**，否则重算值撞 NaN 被值不变早退吞掉。
...
- `native/src/lap_hook.c` / `native/src/lap_hook.h` — time-trial valid-lap listener + paddock upload source: hooks `odometerHandler.HandleSectorsTimes` (lap events) + `IRDSLevelLoadVariables.Awake` (session reset); valid-lap from game's own `validLap` bit; session gate via `champManager.isTimeAttack` (NULL = suspend); track ID via `LAPscene` probe (SceneManagerHelper + GetGPIndex); valid laps exposed to Java via single-slot upload buffer (`lap_poll_upload`/`lap_mark_upload_consumed`). ⚠️ lap completion must fire on the `order==2` event, never on the 2→0 wrap (delayed a full S1). **整圈辅助配置一致性（2026-09-18）**：`g_assist_epoch` 计数器 + 圈边界快照（`LLV.Awake` = 第一圈起点、每次 `order==2` = 上一圈结束即下一圈起点），圈完成时比对 `g_assist_epoch == lap_start_epoch`，不等则五维全填 0 = 缺失。⚠️ **用 epoch 而非逐维比对**："改了又改回"逐维比对会误判一致，epoch 抓得住。⚠️ 比对用的 boundary 值必须**先取到局部变量再更新** `g_assist_boundary_epoch`，顺序颠倒会让比对退化成恒真。开销 = 每圈 1 次整数比较。See `docs/LAP_HOOK_NOTES.md`.
- `native/src/crash_hook.c` / `native/src/crash_hook.h` — 游戏进程 native 崩溃自捕（信号级）：SIGSEGV 等六信号 handler 落盘 `/sdcard/Android/media/<游戏包>/ala_tool_crash_native.log`（PC/LR 相对模块偏移 + 寄存器现场，对拍 OffsetTable 定位元凶 hook）；`sigaltstack`+`SA_ONSTACK` 覆盖栈溢出型；handler 内全 async-signal-safe、链式转发旧 handler；由 `NativeBridge.init` 安装（hook 装之前，尽早覆盖场景加载窗口）。导出走 `LogExporter`「游戏进程崩溃记录」段（media 直读，不走 24h 过滤）。模块进程 Java 侧对应 `util/CrashCatcher.kt`（未捕获异常 → `ala_tool_crash.log`）。
- `app/src/main/kotlin/tools/alamobile/mod/PaddockClient.kt` / `PaddockUploader.kt` — paddock (围场) leader-board client: HTTPS lap upload (HttpURLConnection, no new deps), 30-day local pending queue, login/register/reset API, `fetchMe`（登录态恢复+计时赛总积分）, server Toast mapping; `PaddockUploader` polls native slot at 1Hz from AlaMobileModule's 15s delay path, server address = ModConfig `paddock_server` override (empty → built-in `https://paddock.takotsubo.cloud`). Track display names are a **shared contract** with the paddock repo (see below). **辅助配置上报（2026-09-18）**：有效圈随圈上报当圈的踏板模式 / TC / ABS **原始枚举**（`LapAssist`），服务端派生"关/低/线性"并算加成——模块不报派生结论，调规则不必发模块版本。⚠️ **待传队列必须一并存配置**（`LapAssist.putIntoQueue`），否则"先跑圈后登录"补传时白丢加成。⚠️ 整圈一致性由 native 判定（epoch 快照，见 `native/src/lap_hook.c`），不一致时整组填 0 = 缺失。 ⚠️ **跨包"信箱"通道是配置与登录态的主通道**（2026-09-12，实机验收）：模块 App 持 **AFA（MANAGE_EXTERNAL_STORAGE）** 写 `/sdcard/Android/media/<游戏包>/ala_tool_config.json` + `paddock_auth.json`，游戏进程同 uid 直读**自己包**该目录（零权限）。这是**唯一**在"模块 App 不运行 + ColorOS 关关联启动 + 游戏 stopped state"三重约束下仍可达的通道——不经 AMS（ColorOS 启动管控失效）、不经组件拉起（stopped state 失效）、纯内核文件读写。⚠️ 官方明文：AFA **不覆盖**他包 `Android/data`（实机 AFA 后仍 `ENOENT`），但 `Android/media` 不在受限区（AFA 后读写成功，AFA 前 `EPERM`）——这是整个方案成立的那个缺口。⚠️ **登录态以信箱为权威**：`saveAuth` 写信箱（有 token）；`clearAuth` 写**空 token 而非删文件**——"文件存在但 token 空"= 权威"已登出"（游戏侧据此**否决门控缓存**），"文件不存在"= 从未登录（回落缓存）。这个区分是**反向红线**（"模块未登录、游戏不得凭陈旧缓存放行"）的机制保证。⚠️ `loadAuth` 通道链（游戏进程）：**信箱（存在即权威，命中即 return）→ 本地 auth 文件 → daemon remote prefs → ConfigProvider read_token → 门控 token 缓存**。⚠️ **门控 token 缓存**（`GATE_CACHE_PREFS`，游戏进程内部 prefs）：任何通道恢复成功即写，全通道被拦时兜底；`clearAuth` 的 `clearGateCache()` **只对游戏进程调用方有效**（模块 App 登出清不到游戏进程的 prefs，靠信箱空 token 否决）。⚠️ **跨进程配置新鲜度仲裁**（`readFromTargetProcess`）：四源（remote prefs / ConfigProvider / **信箱** / 游戏本地 externalFilesDir）比 `saved_at` 最新者胜出。**null 源必须先排除再比 ts**——`savedAt(null)` 返回 0（哨兵"最旧"），若不排除，null 源与"无 saved_at 的旧 JSON"同为 0 时首条分支恒真 → `bestJson=null` → **丢弃已读到的配置**（实机：`ConfigProvider ok len=865` 下一行即 `No config ... using defaults`）。⚠️ **模块必须有 AFA**：未授权时 `ConfigActivity` 显示满屏不可跳过的「必要权限」门（`PermissionGateScreen`），`return@setContent` 不组合主界面；`LifecycleEventObserver` 监听 `ON_RESUME` 驱动 key 重查（`LifecycleResumeEffect` 只在首次进入组合时跑，感知不到从外部设置页归来）。⚠️ **门控页必须与主界面走同一套外壳**（2026-09-12 修复）：`PermissionGateScreen` 用共享的 `Scaffold + BlurredBar(毛玻璃 TopAppBar) + LazyColumn(overScrollVertical+scrollEndHaptic) + layerBackdrop` 结构（12dp 页边距/12dp 卡间距，同概览页），且 `ConfigActivity` 的未授权分支要 provide **同一套 CompositionLocal**（`LocalDensity`×pageScale / `LocalColorMode` / `LocalEnableBlur` / 底栏两个 blur 开关 / badge）。⚠️ `enableEdgeToEdge` 的 `DisposableEffect` **必须排在权限门 `return@setContent` 之前**——否则门控页不启用边到边、TopAppBar 的 statusBar inset 与窗口行为叠加出错位间距。改前的老实现只包一层 `MiuixTheme`（只借配色，丢掉 blur/pageScale/边到边）+ 手写裸 Column 布局 = 与主界面视觉断裂。**新增任何"提前 return 的独立分支页"都要照此办理**。⚠️ **登录 token 跨进程通道是三级回落**（loadAuth 顺序）：① 本地 auth 文件（`getExternalFilesDir` 下，`saveAuth` 双写双清）→ ② Remote Preferences daemon（`paddock_token_v1`，游戏进程经 `remoteTokenReader` 读）→ ③ **ConfigProvider `read_token`**（游戏进程 call 模块进程，读同一 auth 文件）。⚠️ **NPatch 本地模式下 ② 是空壳**：loader 的 `requestRemotePreferences` 返回 `Bundle.EMPTY`，模块 App 经管理器写入的 store 与游戏进程本地 fallback store 是两个物理文件——LSPosed 下 ② 权威，NPatch 下 ③ 才是唯一可用通道（用户日志实证 41 次重试全 null）。⚠️ ConfigProvider 读 auth 文件必须与 `saveAuth` 的 `getDir()` 同用 `getExternalFilesDir`，读 `filesDir` 永远 null。⚠️ **登录判定的权威通道是 `queryLoginStateFromModule()`，两级回落**（2026-09-11 修复+实机验收）：① ConfigProvider read_token（NPatch 下进程内必达，权威）→ ② Provider 抛 `Unknown authority`（= LSPosed 下游戏包对模块包无包可见性，官版游戏 manifest 改不了 queries，Binder call 根本没发出去；曾致模块已登录仍被误判未登录、零 hook+循环 Toast）时回落 daemon remote prefs——LSPosed **读侧**权威。⚠️ remote prefs 的不可靠只在**写侧**（remove() 两框架都不落盘，日志成功、DB 不变），读侧没有残留误判问题（清过的 key 读回空串）——"绝不读 remote prefs"旧定案只对写侧成立。清 token 必须 `putString("")` 空串覆盖；token 写/清**与配置写入**（`ModConfig.write`）都必须遍历 `App.allServices`（LSPosed 与 NPatch 是两个物理 store，单 binder 会残留旧 token 或漏写配置；NPatch 无 daemon 异步推 binder，需 `App.xposedService==null` 时主动 `bindNpatchRemoteService`）——单 store 残留 → "退出登录后游戏仍放行+上传 401"（实机 2026-09-12 双框架验证：配置写入同时打出 `(NPatch)` + `(LSPosed)` 两条）。⚠️ **新安装强制清一次登录态**（`wipeAuthIfFreshInstall`）：内部 prefs 标记 `auth_wipe_done_v1`（卸载随删）+ external auth 文件（跨卸载存活）存活期错开构成 fresh-install 指纹，标记缺失 = 卸载重装 → 无条件全渠道清；升级覆盖安装标记在 → 不清。⚠️ **该函数必须只在模块 App 进程执行**（2026-09-14 实机修复，`if (remoteTokenReader != null) return`）：游戏进程没有独立安装周期语义，却持有**自己包**的 `INSTALL_PREFS`；NPatch「清数据」把两进程内部 prefs 一并清掉 → 「清数据」与「卸载重装」物理上不可区分 → 模块 App 登录写好权威信箱（60B）后，游戏进程首次冷启动也判「新安装」并 `clearAuth`，把信箱清成空 token（12B，"存在即权威"→ 误判已登出 = 零 hook + 循环 Toast）。**信箱写方收敛**：写侧只归模块 App（`saveAuth`/`clearAuth`）+ 新增 `flushAuthToMailbox()`（`App.onServiceBind` 与 daemon token flush 同步补写，token 为空时 no-op 不覆盖权威「已登出」）——P0 修「不该写的写坏了」、P1 修「该写的没写全」，同属"存在即权威"设计被两头各撕开一次的缺口。⚠️ `PaddockUploader` 无 token 时 60s 限流重试恢复 + **每 30s 静默补传本地队列**(`drainPendingQueueSilently`,仅当 `pendingCount()>0`;失败不弹 Toast 只落日志)+ 进围场页自动 drain 队列——"先跑圈后登录"/弱网丢圈的圈靠这几条补传。⚠️ **上传失败一律入队 + 提示**(2026-09-10 用户定案):`uploadLap` 的 200 以外**全部**分支(含 4xx/5xx/IOException/意外的非 IO 异常/无 token)都 `enqueue()` 并返回 `TOAST_LAP_QUEUED`(「圈速上传失败,已保存在本地,待连接后自动补传!」),入队与提示一一对应——改前 4xx 与非 IO 异常既不提示也不入队,**圈直接蒸发**(用户反馈"偶尔没见补传、丢了成绩"的根因)。⚠️ 保留/丢弃的**单一判据**是 `isRetryableStatus`(401/408/429/5xx/-1),`uploadLap` 与 `drainQueue` 共用——两处判据曾分裂(5xx 直传当"明确拒绝"丢圈、补传当可重试);永久 4xx 在下一轮补传时丢弃,不占队头挡后面的好圈。⚠️ **鉴权 GET 必须显式传 token**（`getJson(url, token)`——参数默认 null，漏传不会编译报错，只会 401 触发 needRelogin 自动登出，第一次集成 `/v1/me` 就踩过）。⚠️ **两进程共用代码只准用 `Logger`**，`AlaMobileModule.logX` 只准在游戏进程路径用——libxposed-api 是 compileOnly，模块进程没有 Xposed 类，引用即 NoClassDefFoundError（会被外层 catch 捕获伪装成"网络错误"）。⚠️ **头像三级缓存**（fetchAvatar：内存 LruCache→磁盘 cacheDir/paddock_avatars→网络）：失效靠服务端版本化 URL（`?v=avatar_version`，上传即变，URL 同=内容同），**磁盘必须存降采样 JPEG 而非原图字节**——原图最大 ~1MB，5MB 上限在满编榜单下必触发 trim 自删 → 重启全量重下死循环（实机日志实证）。围场页进页自动刷新（isCurrentPage+导航栈归位为键）与下拉刷新共用 `PaddockViewModel.refresh(onDone)`。
- `app/src/main/kotlin/tools/alamobile/mod/ui/screen/paddock/` — Paddock UI page (3rd bottom-bar page, MainScreen `when(page)` 序号 2；页序=概览0/配置1/围场2/设置3): 未登录=通行证核验（用户名+密码共用表单，注册/登录并排，注册走"申请即设密+弹窗复制指令"流，无 verify 步骤，格式校验走本地 Toast 规则逐字复刻服务端 auth.rs）；**未登录不挂 PullToRefresh**（核验页内容不满一屏，LazyColumn 消费不掉下拉位移，全量直达 PullToRefresh 会凭空拉出指示器——列表 composable 提为共用 lambda，仅 loggedIn 分支包 PullToRefresh）；注册弹窗正文为防呆文案（勿改指令文字/勿当入群答案），蓝色按钮=复制指令+openQqGroup 跳群（群号/兜底 URL 常量在 `util/UrlUtils.kt` 与概览页 QQ 群入口共用单源）；已登录=用户信息连体卡（首行头像+粗体用户名，下两行三列统计含计时赛积分 `/v1/me`）+ 计时赛排行榜/大奖赛/娱乐匹配入口 + **围场指南卡** + 页底独立退出登录卡（OverlayDialog 确认）。**围场指南**（`PaddockGuide` + `ui/PaddockGuideDialog.kt`，卡片 `Icons.Rounded.MenuBook`，summary「了解围场的运作机制」）：Markdown 问答正文（是什么/怎么上榜/积分公式与实例/积分为何浮动/是否丢成绩），已读状态存模块 filesDir flag（`paddock_guide_seen.flag`，`GUIDE_VERSION` 递增令旧版重弹，模式同 `EulaManager`）。弹窗**锁死**：`onDismissRequest` 置空 → 点窗口外/返回键均无操作，只有蓝色「我已了解」能关；**读完门控**=正文滚到底（或视口容得下）按钮才从「请先阅读指南」变可点（复用 `MarkdownText` 的外部 `scrollState` 判 `value >= maxValue`）。**自动弹出判据**=「已登录 + 落在围场主页（`isCurrentPage && navAtRoot`，root 非二级页）+ 未读 `!PaddockGuide.isSeen`」；注册后首次登录先跳头像页（backStack 变深）故不误弹，从头像页返回主页才弹——`!uiState.needsAvatar` 是必需的第二道闸（`markAvatarDone()` 与 `navigator.push()` 之间无重组点，单靠 `navAtRoot` 有竞态窗口）。**未登录表单底部两个纯说明弹窗**（都只有蓝色「我已了解」/确认按钮，无表单）：**忘记用户名？**=「查询用户名」说明（群里发 5 字，bot 按群身份回用户名）；**忘记密码？**=重置码说明+重置码/新密码两输入框（`showQueryName`/`showReset` 两个独立 state，各按 `mounted`+`show` 双变量驱动退出动画）。`LeaderboardScreen`（积分/赛道榜，行=排名(前三🥇🥉居中放大50%)+圆形头像+用户名+右对齐圈速连排，水平 16dp 对齐 miuix 标准。**数字列（积分/圈速）与数字名次的等宽必须用 OpenType `tnum`，禁用 `FontFamily.Monospace`**——后者是 `GenericFontFamily("monospace")`，Compose 走 `Typeface.create("monospace", NORMAL)` 即**运行时按族名查询**，命中哪个文件由 ROM 决定；ColorOS 16（一加平板 2 Pro）实测把它重定向到系统 sans（`cmd font dump` 报 DroidSansMono，但实测相邻数字 advance 为 1→15px/5→23px/4→24px，逐项吻合 `SysFont-Regular`；Flyme 12 同样失效）。`tnum` 是**字体自带**的数字变体，不换字体只换数字字形，对"ROM 换成哪个 sans"免疫（ColorOS SysFont / OPSans-En / Roboto 的 GSUB 表都带 tnum），不带该特性时静默忽略、退化为比例数字不更差。实现 = 文件内 `private val TabularDigits = TextStyle(fontFeatureSettings = "tnum")` + `style = MiuixTheme.textStyles.main.merge(TabularDigits)`；**不要挂到主题的 `textStyles` 上**——那会让版本号/档位/偏移量等无关数字一并变等宽，观感突兀（已试过，用户否决）。排查等宽是否生效的手法：取同一行内**相邻的相同数字**，量其 ink 起点像素差（= 该字形 advance，天然抵消 lsb 与居中对齐干扰），等宽字体下不同数字的该值必须相等）：**榜单行必须是 LazyColumn 的 items 而非 Card 内 forEach**（205 行全量组合会在数据帧撞上转场/过渡动画掉帧；连体卡视觉=每行 surfaceContainer 底色+首末行 16dp 圆角拼接+尾部 12dp spacer item）；**筛选卡钉在 PullToRefresh 手势区之外、刷新指示器出现在筛选卡下方**（指示器永远出现在 PullToRefresh 内容最上方——筛选卡若还是列表第一项就随内容下移，指示器只能落在标题下；LazyColumn 加视口顶缘 `clip(topStart/topEnd 16dp)` 防滚动时行被平直截断缺角）；**切换两阶段时序**：旧行显式 `Animatable` alpha 渐隐（立即启动，与筛选卡伸缩并行）→ 完成后换 `visibleBoard` 数据源（渲染数据≠筛选状态）→ 新行 `Modifier.animateItem` 从 0 淡入——animateItem 的移除淡出在换源帧才启动，撞上布局动画结束点就是"闪一下"；头像解码必须 `inSampleSize` 降采样（512px 全尺寸位图×205 张=GC 压力）；页面进入转场 500ms（miuix NavDriver PROGRAMMATIC_DURATION_MILLIS 契约值）结束后才渲染行，数据请求与转场并行不串行。二级页 `navigationIcon` 必须显式 `clickable { navigator.pop() }`（Icon 本身无点击事件）。头像上传（`Route.Avatar`，canhub cropper 1:1 圆形裁剪；`CropImageActivity` 需专用 AppCompat 主题且**必须带 ActionBar**——确认按钮走 options menu）。操作提示全走 Toast 不用页面卡片。Settings 页围场服务器=OverlaySpinnerPreference 选项菜单（CAMDA（默认）/自定义+内联输入框+显式保存按钮）。
...
- `app/src/main/kotlin/tools/alamobile/mod/util/LogExporter.kt` — merge module+game logs（模块 filesDir + 游戏 `/sdcard/Android/media/<游戏包>/`），FileProvider → ShareSheet。**media 直读，无「游戏在运行」门控**（旧 `awaitFreshLogs` 广播探活 + 确认弹窗 + `config/LogReceiver.kt` 推送链已于 2026-09-14 整条删除——门控在游戏闪退时恰好阻止导出崩溃现场）。**另附 NPatch 框架日志**（同目录 `npatch/log/`，按 mtime 取最新 3 个）：覆盖「模块根本没加载起来」的开屏闪退现场；其中**内含 `] Loaded patch config` 的文件被排除**（该行由 LSPApplication 在模块装载时打印 = 本次启动模块真跑过，不可能是崩溃现场），排除后留下的即「启动过但模块从未活下来」的会话；无 `Extracting` 行 ≠ 缓存重建过（那是 `Log.d`/级别过滤不落媒体日志，见 NPATCH_CACHE_CRASH_NOTES §5.1）。
- `app/src/main/kotlin/tools/alamobile/mod/config/CrossPkgMailbox.kt` — 跨包"信箱"通道：`/sdcard/Android/media/<游戏包>/` 读写封装（模块 App 持 AFA 写、游戏同 uid 读自己包目录）。配置与登录态的主传递通道。
...
- `docs/NPATCH_CACHE_CRASH_NOTES.md` — 「游戏启动即崩、关作用域也崩」的用户故障调查记录（**已三次复现：ColorOS 16 / OriginOS 6 各一 + 修复验证**）：根因 = **NPatch 的 origin-apk 缓存无完整性校验/无 fsync/无锁**（`OriginApkHelper.prepareOriginApk:45` 仅 `Files.exists`），坏缓存常驻 → 每次启动以坏文件建 `Resources` → AOSP 静默吞成 null → `LoadedApk.getAssets()` NPE → 系统包成「Unable to instantiate application」。**与模块/PairIP/作用域/重启/Android 版本均无关**（崩溃在模块加载之前——共存版日志里连「游戏进程日志」段都没有，游戏从未跑到 `onPackageReady`）。⚠️ **用户处置 = 清「游戏」的缓存（`CLEAR_CACHE_ONLY`），或卸载重装——不是清 NPatch 管理器的数据**（坏文件在**游戏**的 `dataDir/cache/code_cache/`；清 NPatch 数据无效且有害：会删掉模块 native 缓存、下次启动全量重解压，实测 `Load modules` 从 45ms 涨到 230ms）。判定指纹三条（无 adb 可用）= `cacheApkPath` 恒定 + 连续同栈崩溃 + 全程零 `Extracting`。**用户报同类闪退先读它**；含修正版用户话术 + 给 NPatch 上游的 issue 草稿（#147）。
...
- **`ui/MarkdownText.kt` 是自研轻量 Markdown 渲染器**（不引第三方库）：只支持 `##`/`###` 标题、`**粗体**`、行内代码、无序/有序列表、`---` 分隔线、`[text](url)`；**不支持表格 / 引用块 / 三级以上标题 / 粗斜体（`***`）**——`***x***` 会渲染成「*x*」（外层星号残留）。给这类文案（Release Note、围场指南）排版时按此边界写。可传 `textSizeSp` 覆盖正文字号、传外部 `scrollState` 以做"读完才可点"的门控（见 `PaddockGuideDialog`）。
