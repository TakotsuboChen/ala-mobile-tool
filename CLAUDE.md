...
- `app/src/main/kotlin/tools/alamobile/mod/util/LogExporter.kt` — merge module+game logs（模块 filesDir + 游戏 `/sdcard/Android/media/<游戏包>/`），FileProvider → ShareSheet。**media 直读，无「游戏在运行」门控**（旧 `awaitFreshLogs` 广播探活 + 确认弹窗 + `config/LogReceiver.kt` 推送链已于 2026-09-14 整条删除——门控在游戏闪退时恰好阻止导出崩溃现场）。
