...
- **编辑模式（长按工具图标出入）的几条非显然约定**，改前先读：① **编辑层必须铺满整屏**（`OverlayEditView` 的布局尺寸 = 屏幕，被编辑控件矩形是它内部的 `RectF`）——若让编辑层尺寸 = 控件尺寸，四角圆点圆心落在 view 自身布局边界上、圆面越界被裁成 1/4 个圆，靠父容器 `clipChildren=false` 修不好（父链每层都要放行，实测只修好几个角）。② 铺满整屏后**触摸必须范围闸**：`ACTION_DOWN` 落在控件矩形（含角命中圈）外一律 `return false`，否则整屏失灵。③ **进出编辑模式都必须走 `applyOverlayVisibility()`**（`VISIBLE = editMode \|\| overlaysVisible`）——编辑模式不改 `overlaysVisible`，若某条路径单独按它判定，会出现"退出后控件还留着"或"编辑中改参数控件消失"。④ **退出编辑模式不得重建控件**（旧实现进出都 `removeGamingOverlays()+addGamingOverlays()`，退出时淡出动画作用在刚出生就 GONE 的新层上 = 瞬间恢复无渐亮）。⑤ 编辑模式**屏蔽工具图标单击**（单击=展开/折叠，会与编辑状态打架），唯一进出口都是长按。⑥ 整屏变暗层 z 序**不能插 index 0**（Unity 的 SurfaceView 被合成器提到窗口 View 之上，index 0 在它下面盖不住），必须紧贴游戏内容之后、自家控件之前。⑦ 踏板控件用 `saveLayer` 承担透明度设置，**任何不应跟随透明度的绘制（如编辑模式的「油门」/「刹车」标识）必须放在 `restoreToCount` 之后**。
