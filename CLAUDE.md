...
- M20: 原生 TC/ABS 开关 + 手动换挡 UI 启用 — **TC 已真机验证生效；ABS 未生效（HandleABS 是死代码）；手动换挡待重做**。新增 `enableTc`/`enableAbs` 配置字段（默认 true）全链路同步（ModConfig→ConfigUiState→ConfigReceiver→NativeBridge.setTcAbs→native g_config）。UI: 功能开关卡片新增「牵引力控制」「防抱死」开关，注释掉「自动 DRS」；「手动换挡」去掉「开发中」标签和 `enabled=false`。Native: hook `TractionFilter`（RVA 0x1A570C8）方法入口，关闭时直接返回原始 accel（**TC 已真机验证：关掉后大油门明显更滑**）。hook `HandleABS`（RVA 0x1A5763C）方法入口——**但全 so 搜不到任何 `bl 0x1a5763c` 调用，HandleABS 是死代码，从未被执行，所以 hook 它没用**；后在 FixedUpdate/PlayerControls.Update 写 `absEnable=0` 也无效（没人读这个字段做 ABS 门控）。新增 `DoGearShifting` hook（RVA 0x1A5E734，设 `overrideClutchManagement(0x15C)=1`+`automatic(0xBC)=1` 让 DoGearShifting 开头 return）——**导致车出不了 P 房（起步挂挡也被跳过），已回退为直接调 orig**。手动换挡关自动换挡的正确方式待反汇编确认。AI 车误控（M18 遗留）：`proxy_player_controls_update` 天然只跑玩家车（IRDSPlayerControls 只挂玩家车），但未单独验证。Build + lint green。
