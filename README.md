...
> ⚠️ **仅供个人学习、技术研究与逆向工程教学使用**。本模块会修改目标游戏的运行时行为（内存读写、函数钩子、输入注入），可能违反游戏服务条款并导致账号封禁。请仅在单机模式下使用，并自行承担所有风险。

## 用户协议

首次启动模块设置界面时，会弹出用户协议弹窗，需阅读并点击「同意」后才能使用。同意记录按版本号持久化，后续条款更新（版本号递增）会重新弹窗要求确认；点击「不同意」将退出设置界面。

**条款要点**（完整条款见 App 内弹窗）：

- **仅供学习研究**：仅用于个人学习、技术研究与逆向工程教学，无任何商业目的。
- **免责声明**：软件按「原样」提供，不提供任何明示或暗示担保。
- **责任限制**：对任何直接、间接、附带、特殊或后果性损害（含设备损坏、数据丢失、账号封禁）不承担责任。
- **封号风险**：会修改游戏运行时行为，可能违反服务条款，导致账号暂时或永久封禁；在线/多人模式风险更高。
- **24 小时删除**：下载后 24 小时内仅用于学习研究，之后请删除。
- **禁止行为**：严禁用于商业盈利（代练、刷排名、账号交易等），严禁违反中国相关法律法规。
- **隐私声明**：不收集、不存储、不上传任何个人信息、设备信息或游戏数据；所有配置仅存本地。
- **知识产权**：源代码以 Apache-2.0 开源；游戏名称、素材、商标版权归其各自所有者；与游戏开发商、发行商无任何关联或授权关系。
